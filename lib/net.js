...
function Net(opts) {
    ...
    this.messageListener = socket => this.onNetMessage.bind(this, socket);

    this.debug = debug('swim:net').bind(undefined, opts.debugIdentifier);
    this.trace = debug('swim:trace:net').bind(undefined, opts.debugIdentifier);
}
...
Net.prototype.listen = function listen(callback) {
    this.server = net.createServer( 
        ...
        sock.on('error', this.errorListener);
        sock.on('data', this.messageListener(sock));

    })
    .listen( this.udp.port, this.udp.host, () => {
        
        this.debug('TCP swim server started ' ,this.server.address());
...
Net.prototype.close = function close() {
    this.server.removeListener('error', this.errorListener);
    this.server.close();
};

Net.prototype.onNetMessage = function onNetMessage(rinfo, data) {
    this.debug('received buffer', {
        from: formatRinfo(rinfo),
        length: data.length
    });

    this.onMessage(new Buffer(data.buffer), rinfo);
};
...
Net.prototype.onMessage = function onMessage(buffer, rinfo) {
    ...
    var messageType = Net.ReadMessageType.call(new Uint8Array(buffer), 0);
...
Net.prototype.onPing = function onPing(buffer, rinfo) {
    ...
    this.emit(Net.EventType.Ping, data, formatRinfo(data));
};
...
Net.prototype.onPingReq = function onPingReq(buffer, rinfo) {
    ...
    this.emit(Net.EventType.PingReq, data, formatRinfo(data));
};
...
Net.prototype.onSync = function onSync(buffer, rinfo) {
    ...
    data || this.trace('Data of sync undefined ! ', {
        buffer: buffer.toString(),
        length: buffer.length,
        from: formatRinfo(rinfo),
    });

    this.emit(Net.EventType.Sync, data, formatRinfo(data || {}));
};
...
Net.prototype.onAck = function onAck(buffer, rinfo) {
    ...
    this.emit(Net.EventType.Ack, data, formatRinfo(data));
};
...
Net.prototype.onUpdate = function onUpdate(buffer, rinfo) {
    ...
    data || this.trace('Data of update is undefined ! ', {
        buffer: buffer.toString(),
        length: buffer.length,
        from: formatRinfo(rinfo),
    });

    data && this.emit(Net.EventType.Update, data, formatRinfo(data));
};
...
Net.prototype.sendBuffer = function sendBuffer(buffer, host) {
    ...
    client.on('error', err => this.debug({ msg: `Error contacting ${host}`, err}));
...
function formatRinfo(rinfo) {
    return rinfo.host || rinfo.destination || rinfo.remoteAddress + ':' + rinfo.remotePort;
}
